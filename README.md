...
The API exposes two main endpoints for key-value interaction, both returning **consistent JSON responses** for success and error scenarios.
...
| `value`       | `JSON Object/Array` | The value associated with the key, expected to be a valid JSON object or array.                 | Yes      | `{"name": "Alice", "age": 30, "details": {"city": "Wonderland"}}`                        |
...
- **`400 Bad Request`**
  ...
  - **Response Body (JSON Examples):**
    ...
    {
      "success": false,
      "message": "'value' field must be a valid JSON document"
    }
    ```
...
- **`404 Not Found`**
  ...
  - **Response Body (JSON Example):**
    ...
    {
      "success": false,
      "message": "Key 'temp_cache_item' not found or expired"
    }
    ```
- **`405 Method Not Allowed`**
